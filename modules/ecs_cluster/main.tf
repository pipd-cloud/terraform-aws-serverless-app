...
resource "aws_security_group" "alb" {
  ...
  name        = "${var.id}-${var.container.name}-cluster-alb-sg"
  vpc_id      = var.vpc_id
  tags = merge({
    Name = "${var.id}-${var.container.name}-cluster-alb-sg",
    TFID = var.id
...
resource "aws_vpc_security_group_ingress_rule" "alb_https_public" {
  ...
  cidr_ipv4         = "0.0.0.0/0"
  tags = merge({
    Name = "${var.id}-${var.container.name}-cluster-alb-https-public",
    TFID = var.id
  }, var.aws_tags)
}
...
resource "aws_vpc_security_group_ingress_rule" "alb_https_sg" {
  ...
  tags = merge({
    Name = "${var.id}-${var.container.name}-cluster-alb-https-internal-${data.aws_security_group.internal[count.index].id}",
    TFID = var.id
  }, var.aws_tags)
}
...
resource "aws_vpc_security_group_ingress_rule" "alb_https_pl" {
  ...
  tags = merge({
    Name = "${var.id}-${var.container.name}-cluster-alb-https-${data.aws_security_group.internal[count.index].id}",
    TFID = var.id
  }, var.aws_tags)
}
...
resource "aws_vpc_security_group_ingress_rule" "alb_http_public" {
  ...
  cidr_ipv4         = "0.0.0.0/0"
  tags = merge({
    Name = "${var.id}-${var.container.name}-cluster-alb-sg-http-public",
    TFID = var.id
  }, var.aws_tags)
}
resource "aws_vpc_security_group_ingress_rule" "alb_http_sg" {
  ...
  referenced_security_group_id = data.aws_security_group.internal[count.index].id
  tags = merge({
    name = "${var.id}-${var.container.name}-cluster-alb-http-internal-${data.aws_security_group.internal[count.index].id}",
    TFID = var.id
  }, var.aws_tags)
}
...
resource "aws_vpc_security_group_ingress_rule" "alb_http_pl" {
  ...
  prefix_list_id    = data.aws_prefix_list.internal[count.index].id
  tags = merge({
    name = "${var.id}-${var.container.name}-cluster-alb-http-${data.aws_prefix_list.internal[count.index].id}",
    TFID = var.id
  }, var.aws_tags)
}
...
resource "aws_vpc_security_group_egress_rule" "alb_all" {
  ...
  cidr_ipv4         = "0.0.0.0/0"
  tags = merge({
    Name = "${var.id}-${var.container.name}-cluster-alb-all",
    TFID = var.id
  }, var.aws_tags)
}
